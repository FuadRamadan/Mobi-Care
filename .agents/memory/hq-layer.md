...
- No HQ self-registration by design: first account via the api-server `bootstrap-hq` package script (env vars HQ_ADMIN_USERNAME/PASSWORD). The repo has no `tsx` — one-off TS scripts must be esbuild-bundled, so prefer committed package scripts.
