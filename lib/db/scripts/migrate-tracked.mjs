 ...
 *
 *   --accept-baseline
 *             The baseline was legitimately rewritten — it is a generated
 *             snapshot, so folding a change into it is normal — and this
 *             database should accept the new file as equivalent to the one it
 *             recorded. Updates the stored checksum only. Executes nothing, and
 *             refuses if any numbered migration has also changed, because that
 *             is the drift the check is really there to catch.
 *
...
const MODES = ["--init", "--adopt", "--accept-baseline"];
const modes = process.argv.slice(2).filter((arg) => arg.startsWith("--"));
const unknown = modes.filter((m) => !MODES.includes(m));
if (unknown.length > 0) {
  console.error(`Unknown option(s): ${unknown.join(", ")}. Use ${MODES.join(", ")}, or no option.`);
  process.exit(1);
}
if (modes.length > 1) {
  console.error(`Choose one of ${MODES.join(", ")}, not several.`);
  process.exit(1);
}
...
try {
  ...
  if (mode === "--accept-baseline") {
    const others = drifted.filter((name) => name !== baselineFile);
    if (others.length > 0) {
      console.error(
        `--accept-baseline only re-records the baseline, but these numbered ` +
          `migrations have also changed: ${others.join(", ")}.\n` +
          "A numbered migration that changed after it ran is real drift. " +
          "Resolve it first.",
      );
      process.exit(1);
    }
    if (!applied.has(baselineFile)) {
      console.error(
        "This database has no recorded baseline, so there is nothing to " +
          "accept. Use --init for an empty database, or --adopt for one that " +
          "is already migrated.",
      );
      process.exit(1);
    }
    if (drifted.length === 0) {
      console.log("The recorded baseline already matches the file. Nothing to do.");
    } else {
      await client.query(
        "UPDATE schema_migrations SET checksum = $1 WHERE filename = $2",
        [checksum(await readMigration(baselineFile)), baselineFile],
      );
      console.log(`Accepted the rewritten ${baselineFile}. Nothing was executed.`);
    }
    await client.end();
    process.exit(0);
  }
  ...
  if (drifted.length > 0) {
    const onlyBaseline = drifted.length === 1 && drifted[0] === baselineFile;
    console.error(
      `These migrations changed after they were applied: ${drifted.join(", ")}.\n` +
        (onlyBaseline
          ? "The baseline is a generated snapshot, so this is expected after it " +
            "is regenerated. If the rewrite was deliberate, run with " +
            "--accept-baseline to re-record it; nothing will be executed."
