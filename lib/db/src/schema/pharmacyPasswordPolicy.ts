...
export const pharmacyPasswordPolicyTable = pgTable("pharmacy_password_policy", {
  ...
  temporaryPasswordExpiryHours: integer("temporary_password_expiry_hours").notNull().default(24),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
});

export const insertPharmacyPasswordPolicySchema = createInsertSchema(pharmacyPasswordPolicyTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const selectPharmacyPasswordPolicySchema = createSelectSchema(pharmacyPasswordPolicyTable);

export type InsertPharmacyPasswordPolicy = z.infer<typeof insertPharmacyPasswordPolicySchema>;
export type PharmacyPasswordPolicy = typeof pharmacyPasswordPolicyTable.$inferSelect;
