...
router.get("/", async (req: AuthRequest, res): Promise<void> => {
  ...
  const [searchResult, orderResult, prescriptionResult, searchTrend, orderTrend, pharmacyTrend, patientTrend, topDrugs, topCategories, pharmacyRevenue, categoryRevenue, demand, lowFulfillment, areas, categoryAreas] = await Promise.all([
    ...
    db.execute(sql`SELECT
      ...
      (SELECT coalesce(sum(total_leones), 0) FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end}${pharmacyId ? sql` AND pharmacy_id = ${pharmacyId}` : sql``})::float AS gross_patient_paid,
      (SELECT coalesce(sum(pharmacy_medicine_total_minor), 0) / 100.0 FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end}${pharmacyId ? sql` AND pharmacy_id = ${pharmacyId}` : sql``})::float AS pharmacy_drug_value,
      (SELECT coalesce(sum(medicine_commission_minor), 0) / 100.0 FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end}${pharmacyId ? sql` AND pharmacy_id = ${pharmacyId}` : sql``})::float AS platform_revenue,
    ...
    db.execute(sql`SELECT p.name AS label, (sum(o.medicine_commission_minor) / 100.0)::float AS revenue, count(DISTINCT o.id)::int AS count FROM orders o JOIN pharmacies p ON p.id = o.pharmacy_id WHERE o.completed_at >= ${start} AND o.completed_at <= ${end} AND o.status IN (${values}) GROUP BY p.id, p.name HAVING count(DISTINCT o.id) >= ${MINIMUM_GROUP_SIZE} ORDER BY revenue DESC LIMIT 20`),
    db.execute(sql`SELECT i.primary_category AS label, (sum(oi.base_unit_price_minor * oi.quantity) / 100.0)::float AS revenue, count(DISTINCT o.id)::int AS count FROM orders o JOIN order_items oi ON oi.order_id = o.id JOIN pharmacy_inventory i ON i.id = oi.inventory_id WHERE o.completed_at >= ${start} AND o.completed_at <= ${end} AND o.status IN (${values}) GROUP BY i.primary_category HAVING count(DISTINCT o.id) >= ${MINIMUM_GROUP_SIZE} ORDER BY revenue DESC`),
  ...
  const orderTotals = {
    ...
    ...suppressedTotals(order.completed_orders, { completedOrders: order.completed_orders, grossPatientPaid: order.gross_patient_paid, pharmacyDrugValue: order.pharmacy_drug_value, platformRevenue: order.platform_revenue, averageOrderValue: order.average_order_value }),
  };
  ...
  res.json({
    ...
    suppression: {
      ...
      grossPatientPaid: reportable(order.completed_orders) ? "Reported." : SUPPRESSED,
      pharmacyDrugValue: reportable(order.completed_orders) ? "Reported." : SUPPRESSED,
      platformRevenue: reportable(order.completed_orders) ? "Reported." : SUPPRESSED,
      ...
      platformRevenueByPharmacy: `Groups with fewer than ${MINIMUM_GROUP_SIZE} completed orders are omitted.`,
      drugValueByCategory: `Groups with fewer than ${MINIMUM_GROUP_SIZE} completed orders are omitted.`,
...
router.get("/export.csv", async (req: AuthRequest, res): Promise<void> => {
  ...
  const [searchResult, orderResult, prescriptionResult, searchesByPeriod, ordersByPeriod, pharmaciesByPeriod, patientsByPeriod, drugs, categories, pharmacyRevenue, categoryRevenue, demand, areas, categoryAreas, lowFulfillment] = await Promise.all([
    ...
    db.execute(sql`SELECT (SELECT count(*) FROM orders WHERE created_at >= ${start} AND created_at <= ${end})::int AS order_volume, (SELECT count(*) FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end})::int AS completed_orders, (SELECT coalesce(sum(total_leones), 0) FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end})::float AS gross_patient_paid, (SELECT coalesce(sum(pharmacy_medicine_total_minor), 0) / 100.0 FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end})::float AS pharmacy_drug_value, (SELECT coalesce(sum(medicine_commission_minor), 0) / 100.0 FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end})::float AS platform_revenue, (SELECT coalesce(avg(total_leones), 0) FROM orders WHERE status IN (${values}) AND completed_at >= ${start} AND completed_at <= ${end})::float AS average_order_value`),
    ...
    db.execute(sql`SELECT p.name AS label, (sum(o.medicine_commission_minor) / 100.0)::float AS value FROM orders o JOIN pharmacies p ON p.id=o.pharmacy_id WHERE o.status IN (${values}) AND o.completed_at >= ${start} AND o.completed_at <= ${end}${pharmacyId ? sql` AND o.pharmacy_id = ${pharmacyId}` : sql``} GROUP BY p.id,p.name HAVING count(DISTINCT o.id) >= ${MINIMUM_GROUP_SIZE} ORDER BY value DESC`),
    db.execute(sql`SELECT i.primary_category AS label, (sum(oi.base_unit_price_minor * oi.quantity) / 100.0)::float AS value FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN pharmacy_inventory i ON i.id=oi.inventory_id WHERE o.status IN (${values}) AND o.completed_at >= ${start} AND o.completed_at <= ${end}${pharmacyId ? sql` AND o.pharmacy_id = ${pharmacyId}` : sql``} GROUP BY i.primary_category HAVING count(DISTINCT o.id) >= ${MINIMUM_GROUP_SIZE} ORDER BY value DESC`),
  ...
  for (const [section, result] of Object.entries({ search_trend: searchesByPeriod, order_trend: ordersByPeriod, active_pharmacies_trend: pharmaciesByPeriod, registered_patients_trend: patientsByPeriod, top_drugs: drugs, top_categories: categories, platform_revenue_by_pharmacy: pharmacyRevenue, drug_value_by_category: categoryRevenue, demand_by_category: demand, search_volume_by_area: areas, demand_by_category_area: categoryAreas, high_search_low_fulfillment: lowFulfillment })) {
