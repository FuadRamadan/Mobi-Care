...
router.get("/", async (_req, res) => {
  const couriers = await db.select().from(couriersTable).orderBy(desc(couriersTable.createdAt));

  const loads = couriers.length
    ? await db
        .select({ courierId: ordersTable.courierId, count: sql<number>`count(*)::int` })
        .from(ordersTable)
        .where(inArray(ordersTable.status, ["assigned", "picked_up", "delivering"]))
        .groupBy(ordersTable.courierId)
    : [];
  const loadByCourier = Object.fromEntries(loads.map((l) => [l.courierId, l.count]));
...
router.post("/", async (req: AuthRequest, res) => {
  const body = z.object({
    ...
    vehicleType: z.enum(["motorbike", "bicycle", "car", "van"]).default("motorbike"),
  }).safeParse(req.body);
  ...
  const [existing] = await db.select({ id: couriersTable.id }).from(couriersTable)
    .where(eq(couriersTable.phone, body.data.phone)).limit(1);
  if (existing) { res.status(409).json({ error: "A courier with this phone already exists" }); return; }

  const [created] = await db.insert(couriersTable).values(body.data).returning();
...
router.patch("/:id", async (req: AuthRequest, res) => {
  ...
  const [existing] = await db.select().from(couriersTable)
    .where(eq(couriersTable.id, id)).limit(1);
  if (!existing) { res.status(404).json({ error: "Courier not found" }); return; }
